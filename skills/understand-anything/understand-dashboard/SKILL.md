...
---<!-- Hermes adaptation: `$ARGUMENTS` is a literal placeholder. When this skill is triggered in Hermes, treat the user's message text as the arguments (the text after the skill invocation). -->

> **Hermes 使用说明**：本技能中的 `$ARGUMENTS` 为占位符，Hermes 不会自动替换它。调用时请把用户消息文本当作参数解析（例如 `/skill:understand-chat` 后跟的问题即 `$ARGUMENTS`）；无参数时按默认行为执行。
